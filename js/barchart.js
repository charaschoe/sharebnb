d3.json("data/paris.json").then(function(data) {
    ...
    const width = 1200; // Angepasste Breite für Scrollbereich
    const height = 500;
    const marginTop = 30;
    ...
    const x = d3.scaleBand()
        ...
        .range([marginLeft, formattedData.length * 150]) // Breiterer Bereich für Scrollen
        .padding(0.1); 
    ...
    const svg = d3.select("#chart").append("svg")
        .attr("width", width)
        .attr("height", height)
        .attr("viewBox", `0 0 ${formattedData.length * 150} ${height}`)
